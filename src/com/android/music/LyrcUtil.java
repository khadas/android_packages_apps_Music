...
public class LyrcUtil {
    ...
    MediaPlaybackActivity mediaPlay;
    ...
    public LyrcUtil(MediaPlaybackActivity mediaPlayActivity) {
        mediaPlay = mediaPlayActivity;
    }

    public void RefreshLRC(long current) {
        if (IsLyricExist) {
    ...
    public void ReadLRC(File f) {
        try {
            if (f == null || !f.exists()) {
                IsLyricExist = false;
                lrclist = null;
                mediaPlay.setLRCText(mediaPlay.getString(R.string.no_lyric),true);
            } else {
                ...
                while ((strTemp = br.readLine()) != null) {
                    //Log.d(TAG,"strTemp = "+strTemp+", GetCharset(f):"+GetCharset(f));
